...
/*function Create_Switch1() {
    try {
        ...
        mySwitch.desc = "Velocidad Reducida";
        mySwitch.attachToId("mySwitch");
        mySwitch.onchange = async function (active) {            
            console.log(active);
            signal1 = await RWS.IO.getSignal('Local_IO_0_DI1');
...
/*sync function Set_Pos1(value) {    
    if (value == 1) {
        await RWS.IO.setSignalValue('Local_IO_0_DI1', 0);//The 'Changed' output is set to 0.
    } else {
        await RWS.IO.setSignalValue('Local_IO_0_DI1', 1);//The 'Changed' output is set to 1.
    }
}
...
function Create_Switch2() {
    try{
        ...
        mySwitch2.desc = "Parada";
        mySwitch2.attachToId("mySwitc");
        mySwitch2.onchange = async function (active) {            
            console.log(active);
            signal2 = await RWS.IO.getSignal('Local_IO_0_DI2');
            value2 = await signal2.getValue();   
            console.log(value2);
            if (value2 == 1) {
                await RWS.IO.setSignalValue('Local_IO_0_DI2', 0);//The 'Changed' output is set to 0.
            } else {
                await RWS.IO.setSignalValue('Local_IO_0_DI2', 1);//The 'Changed' output is set to 1.
            }
        }
    } catch (e) { console.log("Something has gone wrong with the Up button!"); } //A popup is displayed if something has gone wrong
}
